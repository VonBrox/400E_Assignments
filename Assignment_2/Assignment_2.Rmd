...
fastqc -o /home/lbrockley_bmeg23/assignment_2/  /projects/bmeg/A2/input_iPSC_SRA66_subset_1.fastq.gz /projects/bmeg/A2/input_iPSC_SRA66_subset_2.fastq.gz
...
scp lbrockley_bmeg23@orca1.bcgsc.ca:/home/lbrockley_bmeg23/assignment_2/input_iPSC_SRA66_subset_1_fastqc.html ./Desktop

#?# Are there any differences between the files for read 1 and read 2? Describe what you see below: - 1.5 pt
## -

These are the same files I analyzed for assignment 1, so my observations are much the same.
Read 1 has superior sequencing quality according to the per sequence quality graph. The higher quality of the first read is a common occurrence and is to be expected.
The quality of data seems relatively high for both fastqs.
Aside from the "per sequence GC content", all sections of the QC report have passed qc. The "per sequence GC content", which has been marked by an orange warning, shows that more than 15% of reads did not have GC content that conformed to the expected normal distribution.
The GC content for both samples is larger than would be predicted by normal distribution, as shown by the rightward shift, aberrant shape, and heightened peak of the curve.
Contamination may explain the irregular forms of the peaks, while systematic bias in thedesign of the study could explicate the notable shift to the right and loftier peaks.
...
**IMPORTANT**: This step can take up to ~30 mins to run, please be mindful of the resources in the server (use htop) 
...
bowtie2 -x /projects/bmeg/indexes/hg38/hg38_bowtie2_index -1 /projects/bmeg/A2/input_iPSC_SRA66_subset_1.fastq.gz -2 /projects/bmeg/A2/input_iPSC_SRA66_subset_2.fastq.gz -S assignment_2/assignment_2_alignment.sam
...
samtools view -h -b -o assignment_2/assignment_2_alignment.bam assignment_2/assignment_2_alignment.sam

#?# What does the -h flag do? Why do you need to include it? - 2 pt

The -h option tells samtools to include the header in the output. I need to include it so that the .bam output will have headers, which will make it easier to read.
...
samtools view assignment_2/assignment_2_alignment.bam | cut -f1,3,4 | head -5
...
sambamba sort assignment_2/assignment_2_alignment.bam 
...
samtools view assignment_2/assignment_2_alignment.sorted.bam | cut -f1,3,4 | head -5

#?# Use the comment character (# at the beginning of the line) to include the output of the command above (one per line): - 0.5pt

#SRR12694366.20112669	chr1	33051659
#SRR12694366.20112669	chr1	33051692
#SRR12694366.10602351	chr1	34566271
#SRR12694366.10602351	chr1	34566271
#SRR12694366.4500457	chr1	42787342


#?# What changed between the two files? Describe what is sambamba sort doing. - 1 pt

The first file started with reads from chromosome 3 with the position numbers arranged in a random order, while the second file starts with  reads from chromosome 1 and has an ascending order of chromosome position numbers. The sambamba sort command has sorted the reads by chromosome number and position.

## If needed, you can inspect more of each bam file (e.g. using `more`, or by increasing the -n parameter for `head` in the above). Just don't bother to include more than 5 lines for each in your submission.
...
sambamba view -F "[XS] == null and not unmapped and not duplicate" -h -f bam -o assignment_2/assignment_2_alignment.sorted.filtered.bam assignment_2/assignment_2_alignment.sorted.bam 

#?# How many reads were there before filtering for uniquely mapped reads? How many are there now?  2 pt

Command:
samtools view assignment_2/assignment_2_alignment.sorted.bam | cut -f1,3,4 | wc -l
Output:
4444490
There were 4444490 reads before filtering.

Command:
samtools view assignment_2/assignment_2_alignment.sorted.filtered.bam | cut -f1,3,4 | wc -l
Output:
3828495
There were 3828495 reads after filtering
...
[Note: the newest basic one for mac is called nano, which has replaced pico]
...
I found that I first had to change the permission of the runTheseJobsSerially.sh file with the command:
chmod u+x runTheseJobsSerially.sh

I was then able to run the command:
./runTheseJobsSerially.sh echo assignment_2/joblist.txt

The output was as follows:
iPSC_input input_iPSC_SRA66_subset_1.fastq.gz input_iPSC_SRA66_subset_2.fastq.gz
iPSC_H3K27me3 H3K27me3_iPSC_SRA60_subset_1.fastq.gz H3K27me3_iPSC_SRA60_subset_2.fastq.gz

The program ./runTheseJobsSerially.sh printed each tab-delimited element in the file assignment_2/joblist.txt to standard output with the echo command.
...
then
        ...
        fastqc -o /home/lbrockley_bmeg23/assignment_2/  /projects/bmeg/A2/$fq1 /projects/bmeg/A2/$fq2
...
./runTheseJobsSerially.sh ./fastqToFilteredBam.sh joblist.txt

#?# What happened? Enter your answer below - 1 pt

After I changed the file permission:
chmod u+x fastqToFilteredBam.sh
I was able to run the command:
./runTheseJobsSerially.sh ./fastqToFilteredBam.sh joblist.txt

This ran paired-end fastQC on the files in /projects/bmeg/A2/ listed in joblist.txt,
which included input_iPSC_SRA66_subset_1.fastq.gz and input_iPSC_SRA66_subset_2.fastq.gz ,
as well as H3K27me3_iPSC_SRA60_subset_1.fastq.gz  and H3K27me3_iPSC_SRA60_subset_2.fastq.gz.

The readout was "Starting analysis of [filename]" with an updating progress report.
fastqc.zip and fastqc.html files were produced for each of the files listed above. A MyLogDirectory was created, containing .done files for both the input files and the H3K27me3 files, indicating that the process ran to completion. 

## --

#?# Now run that same command again. What happened this time? Why? Answer below.  - 2 pts

After running the command a second timeI got the following output:
running pipeline for iPSC_input
Already performed fastqc of iPSC_input
running pipeline for iPSC_H3K27me3
Already performed fastqc of iPSC_H3K27me3

This is because the MyLogDirectory contains a record of the fastqc having already been completed for iPSC_input and 
iPSC_H3K27me3, and the program will end instead of repeating the analysis on the same samples.
...
rm MyLogDirectory/iPSC_H3K27me3.fastqc.done MyLogDirectory/iPSC_input.fastqc.done
./runTheseJobsSerially.sh ./fastqToFilteredBam.sh joblist.txt
...
#?# PLACE THE CONTENTS OF YOUR SCRIPT BELOW
#!/bin/bash
set -e # this makes the whole script exit on any error.
sample=$1
fq1=$2
...
then
        ...
        ls /projects/bmeg/A2/$fq1 /projects/bmeg/A2/$fq2
        bowtie2 -x /projects/bmeg/indexes/hg38/hg38_bowtie2_index -1 /projects/bmeg/A2/$fq1 -2 /projects/bmeg/A2/$fq2 -S $sample.alignment.sam
...
Contributions: Liam Brockley worked on this project alone.
